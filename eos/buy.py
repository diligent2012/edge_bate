...
def crawl_enter_list():
    ...
    try:
        ...
        for key,item in enumerate(info_list):
            ...
            is_crawl = find_buy_refer(sell_id)
            print is_crawl
            if(is_crawl):
                print min_single, max_single, price_buy
                insert_data(price_buy, min_single, max_single, sell_id, date, crawl_date)
                insert_buy_refer(sell_id, date, crawl_date)
...
def insert_data(price, min_single, max_single, sell_id, date, crawl_date):
    try:
        ...
        sql = "INSERT INTO `t_btc_buy` (`price`, `min_single`, `max_single`, `sell_id`, `date`, `crawl_date`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')" % (price, min_single, max_single, sell_id, date, crawl_date)
...
def insert_buy_refer(sell_id, date, crawl_date):
    try:
        conn= MySQLdb.connect(
            host='127.0.0.1',
            port = 3306,
        ...
        cur = conn.cursor()
        cur.execute('set names utf8') #charset set code. it is not nessary now
        sql = "INSERT INTO `t_btc_buy_refer` (`sell_id`, `date`, `crawl_date`) VALUES ('%s', '%s', '%s')" % (sell_id, date, crawl_date)
        #sql = "INSERT INTO `ecs_t_marathon` (`name`, `start_run_time`) VALUES ('%s', '%s')" % (name, start_run_time)
        cur.execute(sql)
        conn.commit()
        cur.close()
...
def find_buy_refer(sell_id):
    try:
        ...
        cur = conn.cursor()
        cur.execute('set names utf8') #charset set code. it is not nessary now
        sql = "SELECT * FROM `t_btc_buy_refer`  WHERE sell_id = '%s'" % (sell_id)
        #sql = "INSERT INTO `ecs_t_marathon` (`name`, `start_run_time`) VALUES ('%s', '%s')" % (name, start_run_time)
        cur.execute(sql)
        conn.commit()
        result = cur.fetchall()

    ...
    try:
        ...
        for key,a_item in enumerate(account_list):
            ...
            for key,s_item in enumerate(binance_symbols): 
                ...
                # 判断当前是应该买入还是卖出
                if(SIDE_BUY == filled_order_item['side']):
                ...
                if(SIDE_SELL == filled_order_item['side']):
...
def auto_buy(client, account, symbol, qty, filled_order, oper_record_log):
    ...
    # 判断是否有利润存在 
    is_allow_buy, buy_rate, oper_record_log = oper_buy_profit(min_price, sell_price)
    oper_record_log += "\nBuy-30、是否有利润: %s 利润率: %s 卖出价格: %s 当前最低价格: %s" % (str(is_allow_buy), str(buy_rate), str(sell_price), str(min_price))

    if is_allow_buy:
        # 计算并获取 触发、止盈价格
        stop_buy_price, buy_price, oper_record_log = oper_stop_buy_price( min_price, sell_price )
...
# 自动卖出程序 code: Sell-*
def auto_sell(client, account, symbol, qty, filled_order):
    # 获取上一次买入价格
    ...
    # 判断是否有利润存在 
    is_allow_sell, sell_rate, oper_record_log = oper_sell_profit(max_price, buy_price)
    oper_record_log += "\nSell-30、是否有利润: %s 利润率: %s 买入价格: %s 当前最高价格: %s" % (str(is_allow_sell), str(sell_rate), str(buy_price), str(max_price))
    
    if is_allow_sell:
        # 计算并获取 触发、止损价格 
        stop_sell_price, sell_price, oper_record_log = oper_stop_sell_price( max_price, buy_price )
